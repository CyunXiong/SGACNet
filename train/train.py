...
def train_main():
    ...
    if args.last_ckpt:
        ckpt_path = os.path.join(ckpt_dir, args.last_ckpt)
        epoch_last_ckpt, best_miou, best_miou_epoch, best_pacc, best_pacc_epoch, best_macc, best_macc_epoch = \
            load_ckpt(model, optimizer, ckpt_path, device)
        start_epoch = epoch_last_ckpt + 1
    else:
        start_epoch = 0
    ...
    # log_keys1 = [f'pacc_{valid_split}']
    # log_keys2 = [f'macc_{valid_split}']
    if args.valid_full_res:
        log_keys.append(f'mIoU_{valid_split}_full-res')
        # log_keys.append(f'pacc_{valid_split}_full-res')
        # log_keys.append(f'macc_{valid_split}_full-res')
        best_miou_full_res = 0
        best_pacc_full_res = 0
        best_macc_full_res = 0

    log_keys_for_csv = log_keys.copy()
    # log_keys_for_csv1 = log_keys1.copy()
    # log_keys_for_csv2 = log_keys2.copy()
    ...
    csvlogger = CSVLogger(log_keys_for_csv,os.path.join(ckpt_dir, 'logs.csv'),
                          append=True)
    # csvlogger = CSVLogger(log_keys_for_csv1,os.path.join(ckpt_dir, 'logs.csv'),
    # append=True)
    # csvlogger= CSVLogger(log_keys_for_csv2,os.path.join(ckpt_dir, 'logs.csv'),
    # append=True)
    ...
    for epoch in range(int(start_epoch), args.epochs):
        ...
        if args.valid_full_res:
            miou_full_res, logs, macc_full_res, pacc_full_res = validate(
        ...
        logs.pop('time', None)
        # csvlogger2.write_logs(logs)
        # csvlogger1.write_logs(logs)
        ...
        save_ckpt_every_epoch(ckpt_dir, model, optimizer, epoch, best_miou,
                              best_miou_epoch, best_pacc,best_pacc_epoch,
                          best_macc,best_macc_epoch)
    ...
    with open(os.path.join(ckpt_dir, 'finished.txt'), 'w') as f:
        ...
        # f.write("Parameters: %s Seed: %s" % (str(total_paramters, GLOBAL_SEED)))#*, GLOBAL_SEED
        if args.valid_full_res:
...
def validate(model, valid_loader, device, cameras, confusion_matrices,
    ...
    for camera in cameras:
        with valid_loader.dataset.filter_camera(camera):
            ...
            # we can compute miou and ious. #?macc and pacc
            #* cm_start_time = time.time()
            ...
            #         pixel_acc += confusion_matrix[i, i] 
            #     pixel_acc = pixel_acc/np.sum(confusion_matrix)
            # ### m acc
            # for i in range(nclasses):
            #     m_acc += confusion_matrix[i, i] /np.sum(confusion_matrix[i, :])
            # m_acc = m_acc/nclasses
    ...
    #  #acc = 0
    # for i in range(n_classes_without_void):
    #     pixel_acc += confusion_matrices[i, i] 
    # pixel_acc = pixel_acc/np.sum(confusion_matrices)
    # ### m acc
    # for i in range(n_classes_without_void):
    #     m_acc += confusion_matrices[i, i] /np.sum(confusion_matrices[i, :])
    # m_acc = m_acc/n_classes_without_void
    miou['all'], ious['all'], pacc['all'], macc['all'] = confusion_matrices['all'].compute_miou()
    cm_time += time.time() - cm_start_time

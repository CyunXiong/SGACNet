...
if __name__ == '__main__':
    ...
    toolbox_dir1 = os.path.join(output_path, 'SUNRGBDtoolbox.zip')
    # download and extract data
    if not os.path.exists(toolbox_dir1):
        download_file(DATASET_TOOLBOX_URL, toolbox_dir1,
                      display_progressbar=True)
    with ZipFile(toolbox_dir1, 'r') as zip_ref:
        zip_ref.extractall(os.path.dirname(toolbox_dir1))
    os.remove(toolbox_dir1)

...
from src.models.rgb_depth_fusion import ESqueezeAndExciteFusionAdd,SqueezeAndExciteFusionAdd
from src.models.context_modules import get_context_module
from src.models.resnet import BasicBlock, NonBottleneck1D,BRUBlock,SS_nbt_module,DABModule
from src.models.model_utils import ConvBNAct, Swish, Hswish


class SGACNet(nn.Module):
    def __init__(self,
                 height=480,
                 width=640,
                 num_classes=37,
                 ...
                 encoder_block='NonBottleneck1D',#BasicBlock,SS_nbt_module
                 channels_decoder=None,  # default: [128, 128, 128]
                 pretrained_on_imagenet=True,
                 pretrained_dir='./trained_models/imagenet',
                 activation='relu',
                 encoder_decoder_fusion='add',
                 context_module='appm',#*sppm,dappm,ppm,appm
                 nr_decoder_blocks=None,  # default: [1, 1, 1]
                 fuse_depth_in_rgb_encoder='SE-add',
                 upsampling='learned-3x3-zeropad'):

        super(SGACNet, self).__init__()
        ...
        # if fuse_depth_in_rgb_encoder == 'SE-add':
        #     self.se_layer0 = ESqueezeAndExciteFusionAdd(
        #         64, activation=self.activation)
        #     self.se_layer1 = ESqueezeAndExciteFusionAdd(
        #         self.encoder_rgb.down_4_channels_out,
        #         activation=self.activation)
        #     self.se_layer2 = ESqueezeAndExciteFusionAdd(
        #         self.encoder_rgb.down_8_channels_out,
        #         activation=self.activation)
        #     self.se_layer3 =ESqueezeAndExciteFusionAdd(
        #         self.encoder_rgb.down_16_channels_out,
        #         activation=self.activation)
        #     self.se_layer4 = ESqueezeAndExciteFusionAdd(
        #         self.encoder_rgb.down_32_channels_out,
        #         activation=self.activation) 
    ...
    # ?  定义我们要提取出得特定层 
        ...
        # all_dict['encoder_rgb.forward_first_conv'] =rgb
        # all_dict['encoder_depth.forward_first_conv'] =depth
        
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth
        else:
            fuse,se_rgb,se_depth = self.se_layer0(rgb, depth)
            # all_dict['se_layer0'] = fuse 
            # all_dict['se_rgb0'] = se_rgb 
            # all_dict['se_depth0'] = se_depth 
        ...
        # all_dict['encoder_rgb.forward_layer1'] =rgb
        # all_dict['encoder_depth.forward_layer1'] =depth
        
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth
        else:
            fuse,se_rgb,se_depth = self.se_layer1(rgb, depth)
            # all_dict['se_layer1'] =fuse 
            # all_dict['se_rgb1'] = se_rgb 
            # all_dict['se_depth1'] = se_depth 
        ...
        # all_dict['encoder_rgb.forward_layer2'] =rgb
        # all_dict['encoder_depth.forward_layer2'] =depth
        
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth
        else:
            fuse,se_rgb,se_depth = self.se_layer2(rgb, depth)
            # all_dict['se_layer2'] =fuse 
            # all_dict['se_rgb2'] = se_rgb 
            # all_dict['se_depth2'] = se_depth 
        ...
        # all_dict['encoder_rgb.forward_layer3'] =rgb
        # all_dict['encoder_depth.forward_layer3'] =depth
        
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth
        else:
            fuse,se_rgb,se_depth = self.se_layer3(rgb, depth)
            # all_dict['se_layer3'] =fuse 
            # all_dict['se_rgb3'] = se_rgb 
            # all_dict['se_depth3'] = se_depth 
        ...
        # all_dict['encoder_rgb.forward_layer4'] =rgb
        # all_dict['encoder_depth.forward_layer4'] =depth
        
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth
        else:
            fuse,se_rgb,se_depth = self.se_layer4(rgb, depth)
            # all_dict['se_layer4'] =fuse 
            # all_dict['se_rgb4'] = se_rgb 
            # all_dict['se_depth4'] = se_depth 

        out = self.context_module(fuse)
        # all_dict['context_module'] = out 
...
class Decoder(nn.Module):
    ...
    def forward(self, enc_outs):
        ...
        # out = self.decoder_module_1(enc_out, enc_skip_down_16)
        # out = self.decoder_module_2(out, enc_skip_down_8)
        # out = self.decoder_module_3(out, enc_skip_down_4)
